...
<form method="POST" action="/commerces/{{$commerce->id}}" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
        <div class="form-group">
            <label for="nomCom">Nom du Commerce</label>
            <input type="text" name="nomCom" class="form-control" id="nomCom" placeholder="Nom du commerce..." value="{{ $commerce->nomCom }}" required>
        </div>
        <div class="form-group">
            <label for="ad1Com">Adresse du Commerce</label>
            <input type="text" name="ad1Com" class="form-control" id="ad1Com" placeholder="Adresse du commerce..." value="{{ $commerce->ad1Com }}" required>
        </div>
    <div class="form-group">
        <label for="ville_id" class="form-label">Choisir une Ville</label>
        <input class="form-control" list="villesList" id="ville_id" name="ville_id" placeholder="Ville a chercher..." value="{{ $commerce->ville->ville_nom}}">
    ...
    <span class="fst-italic"><a class="link-info" href="https://www.coordonnees-gps.fr" target="_blank">Si vous ne connaissez pas vos coordonnées GPS</a></span>
        <div class="form-group">
            ...
            <input type="text" name="latCom" class="form-control" id="latCom" placeholder="Latitude du commerce..." value="{{ $commerce->latCom }}" required>
        </div>
        <div class="form-group">
            <label for="longCom">Longitude du Commerce</label>
            <input type="text" name="longCom" class="form-control" id="longCom" placeholder="Longitude du commerce..." value="{{ $commerce->longCom }}" required>
        </div>
    <div class="form-group">
        <label for="siretCom">Siret du Commerce</label>
        <input type="text" name="siretCom" class="form-control" id="siretCom" placeholder="Siret du commerce..." value="{{ $commerce->siretCom }}" required>
    </div>
        <div class="form-group">
            <label for="telCom">Téléphone du Commerce</label>
            <input type="tel" name="telCom" class="form-control" id="telCom" placeholder="Téléphone du commerce..." value="{{ $commerce->telCom }}" required>
        </div>
        <div class="form-group">
            <label for="mailCom">Mail du Commerce</label>
            <input type="email" name="mailCom" class="form-control" id="mailCom" placeholder="Mail du commerce..." value="{{ $commerce->mailCom }}" required>
        </div>
    <div class="form-group">
        <label for="siteCom">Site du Commerce</label>
        <input type="text" name="siteCom" class="form-control" id="siteCom" placeholder="Site du commerce..." value="{{ $commerce->siteCom }}">
    </div>
        <div class="form-group">
            <label for="descCom">Description du Commerce</label>
            <textarea type="text" name="descCom" class="form-control" id="descCom" placeholder="Description du commerce..." required>{{ $commerce->descCom }}</textarea>
        </div>
        ...
        <div class="form-group" >
            ...
            <select data-role="tagsinput" name="categorie_id" class="form-control" id="categorie_id" required>
                ...
                @foreach ($categories as $category)
                    @if ($category->id === $commerce->categorie_id)
                        <option value="{{$category->id}}" selected="selected">{{$category->libCat}}</option>
                    @else
        ...
        <select data-role="tagsinput" name="sous_categorie_id" class="form-control" id="sous_categorie_id">
            ...
            @foreach ($souscategories as $souscategory)
                @if ($souscategory->id === $commerce->sous_categorie_id)
                    <option data-categoryid="{{ $souscategory->categorie_id }}" value="{{$souscategory->id}}" selected="selected">{{$souscategory->libSousCat}}</option>
                @else
    ...
    <div class="form-group">
        ...
        <textarea type="text" name="descPhoto" class="form-control" id="descPhoto" placeholder="Description de la photo...">
            @foreach($photos as $photo)
                @if($photo->commerce_id === $commerce->id)
                    {{ $photo->descPhoto }}
                @endif
    ...
    <script>
        ...
        $(function(){
            $('#categorie_id').change(function() {
                ...
                $('#sous_categorie_id option[value=0]').attr('selected','selected');
